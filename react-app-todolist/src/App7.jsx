...
function App() {
  ...
  useEffect(() => {
    ...
    fetchTodos().then(setTodos).catch((error) => console.error('error:', e));
  }, []);

  // 計算統計數據
  const totalTodos = todos.length;
  const completedTodos = todos.filter((todo) => todo.completed).length;
  const imcompleteTodos = totalTodos - completedTodos;
  ...
  return (
    <div className='container mt-5'>
      ...
      <div className='mt-4'>
        總數: {totalTodos} 已完成:{completedTodos} 未完成:{imcompleteTodos}
      </div>
    </div>
  )
}
